// Key prefixes for namespace separation in RocksDB
const PREFIX = {
    TX: Buffer.from('tx:'),
...
/** Height index range: all txids at a specific block height */
export function heightIndexRange(height: number): {
    gte: Buffer;
    lt: Buffer;
} {
    const heightBuf = Buffer.alloc(4);
...
/** Height index range: all txids in a block height range [start, end] */
export function heightRangeIndexRange(
    startHeight: number,
    endHeight: number,
